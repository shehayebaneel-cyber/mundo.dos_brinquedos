    ...
    <footer className="mt-12 bg-ink px-4 pb-24 pt-10 text-sm text-white/75 md:pb-10">
      <div className="mx-auto grid max-w-6xl gap-8 sm:grid-cols-2 lg:grid-cols-4">
        <div>
          ...
          <p className="mt-2 leading-relaxed">{settings.tagline}</p>
          <p className="mt-3">📍 {settings.address}</p>
          <p>🕗 {settings.hours}</p>
          <a href={settings.mapsUrl} target="_blank" rel="noreferrer" className="mt-1 inline-block text-sun">Ver no mapa →</a>
        </div>
        <div>
          <h4 className="font-display font-bold text-white">Atendimento</h4>
          <ul className="mt-2 space-y-1.5">
            ...
            <li><Link to="/rastrear" className="hover:text-white">Rastrear pedido</Link></li>
          </ul>
        </div>
        <div>
          <h4 className="font-display font-bold text-white">Institucional</h4>
          <ul className="mt-2 space-y-1.5">
            <li><Link to="/sobre" className="hover:text-white">Sobre nós</Link></li>
            <li><Link to="/atacado" className="hover:text-white">Atacado</Link></li>
            <li><Link to="/entrega" className="hover:text-white">Entrega e frete</Link></li>
            <li><Link to="/trocas" className="hover:text-white">Trocas e devoluções</Link></li>
        ...
        <div>
          <h4 className="font-display font-bold text-white">Siga & pague</h4>
          ...
          <div className="mt-3 flex items-center gap-1.5 text-xs text-white/60">🔒 Compra 100% segura</div>
        </div>
      </div>
      <p className="mx-auto mt-8 max-w-6xl border-t border-white/10 pt-4 text-xs text-white/50">
        © {new Date().getFullYear()} {settings.storeName}. CNPJ 00.000.000/0001-00 · Goiânia, GO. Feito com carinho para as crianças do Brasil.
      </p>
    </footer>
  );
}
