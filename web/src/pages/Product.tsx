...
export function Product() {
  ...
  if (data === null) return <div className="mx-auto max-w-6xl px-4"><Spinner label="Carregando produto…" /></div>;
  const p = data.product;
  ...
  function doAdd(buyNow = false) {
    ...
    cart.add(
      { productId: p.id, slug: p.slug, name: p.name, priceCents, image: p.images[0]?.url ?? "", variant: variantLabel, stock: p.stock },
      qty,
    );
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-5 pb-28 md:pb-5">
      <nav className="mb-3 text-xs text-muted">
        <Link to="/" className="hover:text-brand-dark">Início</Link> ›{" "}
        {p.category && <><Link to={`/categoria/${p.category.slug}`} className="hover:text-brand-dark">{p.category.name}</Link> › </>}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div>
          <div className="relative aspect-square overflow-hidden rounded-[16px] border border-line bg-surface">
            <Thumb url={p.images[imgIdx]?.url} alt={p.name} emojiSize="text-[120px]" />
        ...
        <div>
          ...
          <h1 className="font-display text-2xl font-extrabold text-ink sm:text-3xl">{p.name}</h1>
          ...
          <div className="mt-4">
            ...
            <div className="mt-1 text-sm text-muted tabular">em até {inst.n}x de {brl(inst.eachCents)} sem juros</div>
            <div className="text-sm font-bold text-pix tabular">💠 {brl(pixCents(priceCents, p.pixPercent))} no Pix ({p.pixPercent}% de desconto)</div>
            {p.wholesaleCents && (
              <div className="mt-2 inline-flex items-center gap-2 rounded-lg bg-grape/10 px-3 py-1.5 text-sm font-bold text-grape">
                📦 Atacado: {brl(p.wholesaleCents)} <span className="font-normal text-muted">(mín. {p.minWholesaleQty} un.)</span>
              </div>
            )}
          </div>
          ...
          {Object.entries(kinds).map(([kind, vs]) => (
            <div key={kind} className="mt-4">
              <p className="mb-1.5 text-sm font-bold capitalize text-ink">{kind}: <span className="font-normal text-muted">{picked[kind] ?? "selecione"}</span></p>
          ...
          <div className="mt-4 flex items-center gap-4">
            <span className={`text-sm font-bold ${out ? "text-danger" : low ? "text-warn" : "text-pix"}`}>
              {out ? "● Esgotado" : low ? `● Últimas ${p.stock} unidades` : "● Em estoque"}
            </span>
            {!out && (
          ...
          <div className="mt-4 hidden gap-2 md:flex">
            <button onClick={() => doAdd(false)} disabled={out || missingVariant} className="btn btn-ghost flex-1 py-3 disabled:opacity-50">{added ? "✓ Adicionado!" : "Adicionar ao carrinho"}</button>
            <button onClick={() => doAdd(true)} disabled={out || missingVariant} className="btn btn-primary flex-1 py-3 disabled:opacity-50">Comprar agora</button>
            <button onClick={() => wish.toggle(p.id)} aria-label="Favoritar" className="btn btn-ghost px-4 py-3">{wish.has(p.id) ? "♥" : "♡"}</button>
          </div>
          {missingVariant && <p className="mt-1.5 text-xs font-semibold text-warn">Selecione as opções acima para continuar.</p>}

          <a href={waLink(settings.whatsapp ?? "", `Olá! Tenho interesse no produto "${p.name}" (${brl(priceCents)}). Poderia me ajudar?`)} target="_blank" rel="noreferrer" className="btn mt-2 w-full border border-[#25d366] py-2.5 text-[#128c4a]">💬 Tirar dúvida no WhatsApp</a>

          {/* SHIPPING */}
          <form onSubmit={calcShip} className="mt-4 rounded-[16px] border border-line bg-surface p-4">
            <p className="text-sm font-bold text-ink">🚚 Calcular frete e prazo</p>
            <div className="mt-2 flex gap-2">
              <input value={cep} onChange={(e) => setCep(e.target.value)} placeholder="Digite seu CEP" className="flex-1 rounded-lg border border-line px-3 py-2 text-sm" />
              <button className="btn btn-ghost px-4 py-2 text-sm">Calcular</button>
            </div>
            {ship && (
              <ul className="mt-3 space-y-1.5 text-sm">
                {ship.map((o) => (
                  <li key={o.label} className="flex justify-between"><span>{o.label} <span className="text-muted">· {o.days}</span></span><span className="font-bold tabular">{o.price}</span></li>
                ))}
                <li className="pt-1 text-xs text-muted">Retirada grátis na loja em Goiânia disponível.</li>
              </ul>
            )}
          </form>
        </div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <section className="rounded-[16px] border border-line bg-surface p-5">
          <h2 className="font-display text-lg font-extrabold text-ink">Descrição</h2>
          <p className="mt-2 whitespace-pre-line leading-relaxed text-ink/90">{p.description}</p>
        </section>
        <section className="rounded-[16px] border border-line bg-surface p-5">
          <h2 className="font-display text-lg font-extrabold text-ink">Informações do produto</h2>
          <dl className="mt-2 divide-y divide-line text-sm">
            {[
              ["Marca", p.brand],
              ["Idade recomendada", p.ageGroup],
              ["Material", p.material],
              ["Peso", p.weightGrams ? `${(p.weightGrams / 1000).toFixed(2)} kg` : ""],
              ["Garantia", p.warranty],
              ["Código (SKU)", p.sku],
            ].filter(([, v]) => v).map(([k, v]) => (
      ...
      {data.related.length > 0 && (
        <section className="mt-10">
          <SectionHead title="Você também pode gostar" emoji="🧡" />
      ...
      {!out && (
        <div className="fixed inset-x-0 bottom-[57px] z-20 flex items-center gap-2 border-t border-line bg-surface/95 p-2.5 backdrop-blur md:hidden">
          ...
          <button onClick={() => doAdd(true)} disabled={missingVariant} className="btn btn-primary ml-auto flex-1 py-3 disabled:opacity-50">{missingVariant ? "Escolha as opções" : "Comprar agora"}</button>
          <button onClick={() => doAdd(false)} disabled={missingVariant} className="btn btn-ghost px-4 py-3 disabled:opacity-50" aria-label="Adicionar">🛒</button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <section className="mt-8 rounded-[16px] border border-line bg-surface p-5">
      <div className="flex flex-wrap items-center gap-3">
        <h2 className="font-display text-lg font-extrabold text-ink">Avaliações</h2>
      ...
      <div className="mt-4 space-y-3">
        {reviews.length === 0 && <p className="text-sm text-muted">Ainda não há avaliações. Seja o primeiro!</p>}
        {reviews.map((r) => (
          <div key={r.id} className="border-b border-line pb-3 last:border-0">
            <div className="flex items-center gap-2">
              <span className="font-bold text-ink">{r.author}</span>
              {r.verified && <span className="rounded-full bg-pix/10 px-2 py-0.5 text-[10px] font-bold text-pix">✓ Compra verificada</span>}
            </div>
      ...
      <div className="mt-5 border-t border-line pt-4">
        <h3 className="font-display font-bold text-ink">Avaliar este produto</h3>
        {sent ? (
          <p className="mt-2 font-semibold text-pix">Obrigado! Sua avaliação será publicada após aprovação. 💛</p>
        ) : (
          <form onSubmit={submit} className="mt-2 space-y-2">
            ...
            <input value={form.author} onChange={(e) => setForm((f) => ({ ...f, author: e.target.value }))} placeholder="Seu nome" className="w-full rounded-lg border border-line px-3 py-2 text-sm" />
            <textarea value={form.comment} onChange={(e) => setForm((f) => ({ ...f, comment: e.target.value }))} rows={3} placeholder="Conte o que achou do produto" className="w-full rounded-lg border border-line px-3 py-2 text-sm" />
            <button disabled={busy} className="btn btn-primary px-5 py-2.5 disabled:opacity-60">{busy ? "Enviando…" : "Enviar avaliação"}</button>
          </form>
        )}
      </div>
    </section>
