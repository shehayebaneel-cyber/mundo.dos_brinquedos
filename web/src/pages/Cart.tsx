...
export function Cart() {
  ...
  if (cart.lines.length === 0) {
    return (
      <div className="mx-auto max-w-md px-4 py-16 text-center">
        ...
        <h1 className="mt-3 font-display text-2xl font-extrabold text-ink">Seu carrinho está vazio</h1>
        <p className="mt-1 text-muted">Que tal dar uma olhada nas nossas ofertas?</p>
        <Link to="/produtos" className="btn btn-primary mt-5 px-6 py-3">Ver produtos</Link>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-5">
      <h1 className="mb-4 font-display text-2xl font-extrabold text-ink">Meu carrinho ({cart.count})</h1>
        ...
        <div className="space-y-3">
          {cart.lines.map((l) => (
            <div key={`${l.productId}-${l.variant}`} className="flex gap-3 rounded-[16px] border border-line bg-surface p-3">
              ...
              <button onClick={() => cart.remove(l.productId, l.variant)} className="self-start text-muted hover:text-danger" aria-label="Remover">✕</button>
            </div>
          ))}
        </div>

        <aside className="h-fit rounded-[16px] border border-line bg-surface p-4 lg:sticky lg:top-40">
          <h2 className="font-display font-bold text-ink">Resumo</h2>
          <div className="mt-3 space-y-1.5 text-sm">
            ...
            <div className="flex justify-between"><span className="text-muted">Frete</span><span className="text-muted">calculado no checkout</span></div>
          </div>
          <div className="mt-3 flex justify-between border-t border-line pt-3 font-display text-lg font-extrabold text-ink">
            <span>Total</span><span className="tabular">{brl(cart.subtotalCents)}</span>
          </div>
          <Link to="/checkout" className="btn btn-primary mt-4 w-full py-3">Finalizar compra</Link>
