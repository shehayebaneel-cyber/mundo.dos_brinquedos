  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      <div className="rounded-[16px] bg-gradient-to-br from-mint to-sky p-6 text-center text-white">
        ...
        <h1 className="mt-2 font-display text-2xl font-extrabold">{t("Pedido confirmado!")}</h1>
        <p className="mt-1 text-white/90">{t("Obrigado, {name}! Recebemos seu pedido.", { name: order.customerName.split(" ")[0] })}</p>
      ...
      {order.paymentMethod === "pix" && order.paymentStatus === "pendente" && (
        <div className="mt-4 rounded-[16px] border border-pix/40 bg-pix/5 p-4 text-center">
          <p className="font-bold text-pix">{t("💠 Pague com Pix para liberar seu pedido")}</p>
          <div className="mx-auto mt-3 grid h-40 w-40 place-items-center rounded-xl bg-white text-6xl shadow-inner">▪️</div>
          <p className="mt-2 text-xs text-muted">{t("Escaneie o QR Code no app do seu banco. (No site real, o Pix é gerado pelo Mercado Pago.)")}</p>
        </div>
      )}
      {order.paymentMethod === "boleto" && (
        <div className="mt-4 rounded-[16px] border border-line bg-surface p-4 text-center text-sm">
          {t("🧾 Seu boleto foi gerado e enviado por e-mail. Vence em 3 dias úteis.")}
        </div>
      )}
      ...
      <div className="mt-4 rounded-[16px] border border-line bg-surface p-4">
        ...
        <div className="mt-3 space-y-1 border-t border-line pt-3 text-sm">
          <div className="flex justify-between"><span className="text-muted">{t("Subtotal")}</span><span className="tabular">{brl(order.subtotalCents)}</span></div>
          {order.discountCents > 0 && <div className="flex justify-between text-pix"><span>{t("Desconto")}</span><span className="tabular">- {brl(order.discountCents)}</span></div>}
          <div className="flex justify-between"><span className="text-muted">{t("Frete")}</span><span className="tabular">{order.shippingCents === 0 ? t("Grátis") : brl(order.shippingCents)}</span></div>
          <div className="flex justify-between font-display text-lg font-extrabold text-ink"><span>{t("Total")}</span><span className="tabular">{brl(order.totalCents)}</span></div>
        </div>
      </div>

      <div className="mt-4 rounded-[16px] border border-line bg-surface p-4 text-sm">
        <h2 className="font-display font-bold text-ink">{t("Entrega")}</h2>
        <p className="mt-1 text-muted">{order.address}</p>
        <p className="text-muted">{order.city} - {order.state} · CEP {order.cep}</p>
        <p className="mt-1 text-ink">{t("Previsão: 5 a 9 dias úteis após a confirmação do pagamento.")}</p>
      </div>

      <div className="mt-4 flex flex-col gap-2 sm:flex-row">
        <Link to={`/rastrear?code=${order.code}`} className="btn btn-primary flex-1 py-3">{t("Acompanhar pedido")}</Link>
        <a href={waLink(settings.whatsapp ?? "", `${order.code}`)} target="_blank" rel="noreferrer" className="btn flex-1 border border-[#25d366] py-3 text-[#128c4a]">{t("💬 Falar no WhatsApp")}</a>
      </div>
