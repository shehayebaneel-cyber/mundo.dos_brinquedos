...
export function Shop({ mode }: { mode?: "ofertas" | "search" | "all" }) {
  ...
  const Filters = (
    <div className="space-y-4 text-sm">
      <div>
        <p className="mb-2 font-display font-bold text-ink">{t("Ordenar por")}</p>
      ...
      <div>
        <p className="mb-2 font-display font-bold text-ink">{t("Filtrar")}</p>
        <label className="flex items-center gap-2 py-1"><input type="checkbox" checked={onlyStock} onChange={(e) => setOnlyStock(e.target.checked)} className="accent-brand" /> {t("Em estoque")}</label>
        <label className="flex items-center gap-2 py-1"><input type="checkbox" checked={onlyDiscount} onChange={(e) => setOnlyDiscount(e.target.checked)} className="accent-brand" /> {t("Com desconto")}</label>
        <label className="flex items-center gap-2 py-1"><input type="checkbox" checked={onlyNew} onChange={(e) => setOnlyNew(e.target.checked)} className="accent-brand" /> {t("Novidades")}</label>
      </div>
      <div>
        <p className="mb-1 font-display font-bold text-ink">{t("Preço máximo")}</p>
        <div className="flex items-center gap-2">
          <span className="text-muted">R$</span>
          <input type="number" min={0} value={maxPrice || ""} onChange={(e) => setMaxPrice(Number(e.target.value))} placeholder={t("qualquer")} className="w-full rounded-lg border border-line bg-surface px-3 py-2" />
        </div>
      </div>
    </div>
  );
      ...
      <div className="grid gap-6 lg:grid-cols-[220px_1fr]">
        ...
        <div>
          <div className="mb-3 flex items-center justify-between">
            ...
            <button onClick={() => setFiltersOpen(true)} className="btn btn-ghost px-4 py-2 text-sm lg:hidden">{t("⚙️ Filtros")}</button>
          </div>
      ...
      {filtersOpen && (
        <div className="fixed inset-0 z-40 lg:hidden" onClick={() => setFiltersOpen(false)}>
          ...
          <div className="absolute inset-x-0 bottom-0 max-h-[80vh] overflow-y-auto rounded-t-[1.5rem] bg-surface p-5" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between">
              <span className="font-display text-lg font-extrabold text-ink">{t("Filtros")}</span>
              <button onClick={() => setFiltersOpen(false)} className="text-xl">✕</button>
            </div>
            {Filters}
            <button onClick={() => setFiltersOpen(false)} className="btn btn-primary mt-4 w-full py-3">{t("Ver {n} produtos", { n: filtered.length })}</button>
          </div>
        </div>
      )}
    </div>
