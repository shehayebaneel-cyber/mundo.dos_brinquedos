...
export function AdminReviews() {
  ...
  return (
    <div>
      <h1 className="mb-1 font-display text-2xl font-extrabold text-ink">Avaliações</h1>
      <p className="mb-4 text-sm text-muted">{pending.length} aguardando aprovação de {list.length} no total.</p>
        ...
        {list.map((r) => (
          <div key={r.id} className={`rounded-[16px] border bg-surface p-4 ${r.approved ? "border-line" : "border-warn/50 bg-warn/5"}`}>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`ml-auto rounded-full px-2 py-0.5 text-[11px] font-bold ${r.approved ? "bg-pix/10 text-pix" : "bg-warn/10 text-warn"}`}>{r.approved ? "publicada" : "pendente"}</span>
            </div>
            <p className="mt-1 text-sm text-ink/90">{r.comment}</p>
            <div className="mt-2 flex gap-2">
              {!r.approved && <button onClick={() => approve(r, true)} className="btn btn-primary px-4 py-1.5 text-sm">Aprovar</button>}
              {r.approved && <button onClick={() => approve(r, false)} className="btn btn-ghost px-4 py-1.5 text-sm">Ocultar</button>}
              <button onClick={() => del(r)} className="btn btn-ghost px-4 py-1.5 text-sm text-danger">Excluir</button>
            </div>
          </div>
        ))}
      </div>
