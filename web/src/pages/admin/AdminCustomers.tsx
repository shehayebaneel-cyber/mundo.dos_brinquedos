...
export function AdminCustomers({ wholesaleOnly = false }: { wholesaleOnly?: boolean }) {
  ...
  return (
    <div>
      <h1 className="mb-4 font-display text-2xl font-extrabold text-ink">{wholesaleOnly ? t("Atacado") : t("Clientes")}</h1>
        ...
        {shown.map((c) => (
          <div key={c.id} className="rounded-[16px] border border-line bg-surface p-4">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {c.kind === "atacado" && <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${wsTone[c.wholesaleStatus]}`}>{t(c.wholesaleStatus)}</span>}
            ...
            {c.kind === "atacado" && c.wholesaleStatus !== "approved" && (
              <div className="mt-2 flex gap-2">
                <button onClick={() => setWs(c, "approved")} className="btn btn-primary px-4 py-1.5 text-sm">{t("Aprovar atacado")}</button>
                {c.wholesaleStatus !== "rejected" && <button onClick={() => setWs(c, "rejected")} className="btn btn-ghost px-4 py-1.5 text-sm text-danger">{t("Rejeitar")}</button>}
              </div>
            )}
            {c.wholesaleStatus === "approved" && <button onClick={() => setWs(c, "rejected")} className="btn btn-ghost mt-2 px-4 py-1.5 text-sm">{t("Suspender acesso atacado")}</button>}
          </div>
        ))}
      </div>
    </div>
