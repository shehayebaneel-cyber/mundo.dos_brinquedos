      ...
      <div className="overflow-x-auto rounded-[16px] border border-line bg-surface">
        <table className="w-full min-w-[640px] text-sm">
          <thead className="border-b border-line bg-surface-2 text-left text-xs uppercase text-muted">
            <tr><th className="p-3">{t("Pedido")}</th><th className="p-3">{t("Cliente")}</th><th className="p-3">{t("Total")}</th><th className="p-3">{t("Pgto")}</th><th className="p-3">{t("Status")}</th></tr>
          </thead>
          <tbody>
            {orders.map((o) => (
              <tr key={o.id} onClick={() => setOpen(o)} className="cursor-pointer border-b border-line last:border-0 hover:bg-surface-2">
                <td className="p-3"><div className="font-mono font-bold text-ink">{o.code}</div><div className="text-xs text-muted">{new Date(o.createdAt).toLocaleDateString(t("pt-BR"))}{o.kind === "atacado" && ` · 📦 ${t("atacado")}`}</div></td>
                ...
                <td className="p-3"><span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${tone(o.paymentStatus)}`}>{t(o.paymentStatus)}</span></td>
                <td className="p-3"><span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${tone(o.status)}`}>{t(STATUS_LABEL[o.status])}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-0 sm:items-center sm:p-4" onClick={() => setOpen(null)}>
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-t-2xl bg-surface p-5 sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <div><label className="mb-1 block text-xs font-bold text-muted">{t("Status do pedido")}</label>
              ...
              <div><label className="mb-1 block text-xs font-bold text-muted">{t("Pagamento")}</label>
                <select value={open.paymentStatus} onChange={(e) => update(open, { paymentStatus: e.target.value })} className="w-full rounded-lg border border-line px-3 py-2 text-sm">
                  {PAY_STATUSES.map((s) => <option key={s} value={s}>{t(s)}</option>)}
                </select></div>
              <div className="sm:col-span-2"><label className="mb-1 block text-xs font-bold text-muted">{t("Código de rastreio")}</label>
